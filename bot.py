...
USER_LIVE: Dict[int, bool] = {}                        # live on/off
USER_SEEN: Dict[int, Dict[str, str]] = {}              # seen balances (key -> "bank|balance")
USER_STATS: Dict[int, Dict[str, int]] = {}             # alert count
...
async def show_cmd(u, c):
    ...
    # Populate seen data to avoid duplicate live alerts for existing balances
    ...
    if state == "delete":
        ...
        try:
            ...
            if 0 <= idx < len(urls):
                ...
                # Remove seen entries for this url
                if uid in USER_SEEN:
...
# ============================================================
# ✅ LIVE SCANNER — naye bank message pe turant alert
# ============================================================
def _update_seen_from_results(uid: int, results: List[Dict[str, Any]]):
    """Pehle se jo balance hain unhe 'seen' mark karo (no duplicate alerts)"""
...
async def live_scanner(app):
    """Background task — har LIVE_SCAN_INTERVAL sec me scan karo"""
